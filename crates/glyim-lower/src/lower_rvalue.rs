...
impl<'a> MirBuilder<'a> {
    ...
    pub fn lower_stmt(&mut self, stmt: &thir::Stmt) {
        match stmt {
            thir::Stmt::Let { name, ty, init, span, pat } => {
                let init_local = if let Some(init_expr) = init {
                    let temp_local = self.alloc_local(*ty, glyim_core::primitives::Mutability::Mut, *span);
                ...
                if let thir::PatternKind::Binding { name: bind_name, .. } = &pat.kind {
            ...
            thir::Stmt::Expr { expr } => {
                ...
                let temp = self.alloc_local(expr.ty, glyim_core::primitives::Mutability::Mut, expr.span);
        ...
        match &expr.kind {
            ...
            thir::ExprKind::Ref { mutability, operand } => {
                ...
                let borrow_kind = match mutability {
                    glyim_core::primitives::Mutability::Mut => glyim_mir::BorrowKind::Mut { allow_two_phase_borrow: false },
            ...
            thir::ExprKind::While { cond, body } => {
                ...
                self.terminate(glyim_mir::TerminatorKind::Goto { target: header_bb }, expr.span);
                ...
                if self.current_block.is_some() {
                    self.terminate(glyim_mir::TerminatorKind::Goto { target: header_bb }, body.span);
                }
            ...
            thir::ExprKind::Loop { body } => {
                ...
                let exit_bb = self.new_block();
                self.terminate(glyim_mir::TerminatorKind::Goto { target: loop_bb }, expr.span);
                self.loop_stack.push(LoopInfo {
                    continue_bb: loop_bb,
                    break_bb: exit_bb,
                });
                ...
                self.loop_stack.pop();
                if self.current_block.is_some() {
                    self.terminate(glyim_mir::TerminatorKind::Goto { target: loop_bb }, body.span);
                }
                self.current_block = Some(exit_bb);
                glyim_mir::Rvalue::Use(glyim_mir::Operand::Constant(glyim_mir::MirConst {
                    kind: glyim_mir::MirConstKind::Unit,
            ...
            thir::ExprKind::For { pat, iterable, body } => {
                let iter_ty = iterable.ty;
                let elem_ty = pat.ty;
                let iter_local = self.alloc_local(iter_ty, glyim_core::primitives::Mutability::Mut, iterable.span);
                self.push_stmt(glyim_mir::StatementKind::StorageLive(iter_local), iterable.span);
                let iter_rvalue = self.lower_expr_to_rvalue(iterable);
                self.push_stmt(
                    glyim_mir::StatementKind::Assign(glyim_mir::Place::new(iter_local), iter_rvalue),
                    iterable.span,
                );
                let header_bb = self.new_block();
                let exit_bb = self.new_block();
                ...
                if let Some(iter_info) = self.ctx.iterator_next_fn(iter_ty, elem_ty) {
                    self.current_block = Some(header_bb);
                    let ref_iter_local =
                        self.alloc_local(iter_info.ref_iter_ty, glyim_core::primitives::Mutability::Mut, iterable.span);
                    self.push_stmt(glyim_mir::StatementKind::StorageLive(ref_iter_local), iterable.span);
                    self.push_stmt(
                        glyim_mir::StatementKind::Assign(
                            glyim_mir::Place::new(ref_iter_local),
                            glyim_mir::Rvalue::Ref(
                                glyim_mir::Place::new(iter_local),
                                glyim_mir::BorrowKind::Mut { allow_two_phase_borrow: false },
                            ),
                        ),
                        iterable.span,
                    );
                    ...
                    let ref_iter_op = glyim_mir::Operand::Copy(glyim_mir::Place::new(ref_iter_local));
                    let option_local = self.alloc_local(iter_info.option_ty, glyim_core::primitives::Mutability::Mut, expr.span);
                    self.push_stmt(glyim_mir::StatementKind::StorageLive(option_local), expr.span);
                    ...
                    let none_bb = exit_bb;
                    let switch_targets = glyim_mir::SwitchTargets::new(Box::new([(1, some_bb)]), none_bb);
                    self.terminate(
                        glyim_mir::TerminatorKind::SwitchInt {
                            discr: discr_op,
                    ...
                    let payload_place = {
                        let mut proj = vec![glyim_mir::ProjectionElem::Downcast(glyim_mir::VariantIdx::from_raw(1))];
                        proj.push(glyim_mir::ProjectionElem::Field(FieldIdx::from_raw(0)));
                        glyim_mir::Place { local: option_local, projection: proj.into_boxed_slice() }
                    };
                    let payload_local = self.alloc_local(elem_ty, glyim_core::primitives::Mutability::Not, expr.span);
                    self.push_stmt(glyim_mir::StatementKind::StorageLive(payload_local), expr.span);
                    self.push_stmt(
                        glyim_mir::StatementKind::Assign(
            ...
            thir::ExprKind::Field { receiver, field, ty: _field_ty } => {
                ...
                let field_idx = match field_idx {
                    ...
                    None => {
                        return glyim_mir::Rvalue::Use(glyim_mir::Operand::Constant(
                            glyim_mir::MirConst { kind: glyim_mir::MirConstKind::Error, ty: *_field_ty, span: expr.span },
                        ));
                    }
                };
            ...
            thir::ExprKind::Index { base, index } => {
                ...
                let index_local = self.alloc_local(index.ty, glyim_core::primitives::Mutability::Not, index.span);
                let index_rval = self.lower_expr_to_rvalue(index);
                self.push_stmt(
                    glyim_mir::StatementKind::Assign(glyim_mir::Place::new(index_local), index_rval),
                    index.span,
                );
                let place = self.place_with_projection(base_place, ProjectionElem::Index(index_local));
                glyim_mir::Rvalue::Use(glyim_mir::Operand::Copy(place))
            }
            thir::ExprKind::Cast { expr: inner } => {
                ...
                let cast_kind = match (self.ctx.ty_ctx().ty_kind(inner_ty), self.ctx.ty_ctx().ty_kind(target_ty)) {
            ...
            thir::ExprKind::Struct { adt_id, variant_idx, fields, spread: _ } => {
            ...
            thir::ExprKind::Break { value } => {
                ...
                if let Some(target) = target_bb {
                    ...
                    self.diagnostics.push(GlyimDiagnostic::type_error(expr.span, "break outside of loop".to_string()));
                    self.terminate(glyim_mir::TerminatorKind::Unreachable, expr.span);
                }
                glyim_mir::Rvalue::Use(glyim_mir::Operand::Constant(glyim_mir::MirConst {
                    kind: glyim_mir::MirConstKind::Unit, ty: Ty::NEVER, span: expr.span,
                }))
            }
            thir::ExprKind::Continue => {
                ...
                if let Some(target) = target_bb {
                    ...
                    self.diagnostics.push(GlyimDiagnostic::type_error(expr.span, "continue outside of loop".to_string()));
                    self.terminate(glyim_mir::TerminatorKind::Unreachable, expr.span);
                }
                glyim_mir::Rvalue::Use(glyim_mir::Operand::Constant(glyim_mir::MirConst {
                    kind: glyim_mir::MirConstKind::Unit, ty: Ty::NEVER, span: expr.span,
            ...
            thir::ExprKind::Closure { body: _thir_body, captures } => {
                let (closure_id, closure_substs) = match self.ctx.ty_ctx().ty_kind(expr.ty) {
                    TyKind::Closure(id, substs) => (id, substs),
                    _ => {
                        self.diagnostics.push(GlyimDiagnostic::type_error(expr.span, "closure expression has non-closure type".to_string()));
                        return glyim_mir::Rvalue::Use(glyim_mir::Operand::Constant(
                            glyim_mir::MirConst { kind: glyim_mir::MirConstKind::Error, ty: expr.ty, span: expr.span },
                        ));
                    }
                ...
                for capture in captures {
                    ...
                    let operand = match capture.kind {
                        thir::CaptureKind::ByValue => glyim_mir::Operand::Move(glyim_mir::Place::new(capture_local)),
                        thir::CaptureKind::ByRef(glyim_core::primitives::Mutability::Not) | thir::CaptureKind::ByRef(glyim_core::primitives::Mutability::Mut) => {
            ...
            thir::ExprKind::Err => {
                self.diagnostics.push(GlyimDiagnostic::new(
                    glyim_diag::ErrorCode { category: glyim_diag::ErrorCategory::Internal, number: 0 },
                    glyim_diag::DiagSeverity::Warning,
                    "Err expression in THIR during lowering".to_string(),
                    glyim_diag::MultiSpan::from_span(expr.span),
                ));
                glyim_mir::Rvalue::Use(glyim_mir::Operand::Constant(glyim_mir::MirConst {
                    kind: glyim_mir::MirConstKind::Error, ty: expr.ty, span: expr.span,
                }))
            }
        }
    }
    ...
    pub fn lower_expr_to_operand(&mut self, expr: &thir::Expr) -> glyim_mir::Operand {
        match &expr.kind {
            thir::ExprKind::Literal(_) | thir::ExprKind::FnRef(_) => {
                if let glyim_mir::Rvalue::Use(op) = self.lower_expr_to_rvalue(expr) { op } else { unreachable!() }
            }
            thir::ExprKind::VarRef(var_id) => {
                let local = LocalIdx::from_raw(var_id.to_raw());
            ...
            _ => {
                let rvalue = self.lower_expr_to_rvalue(expr);
                let local = self.alloc_local(expr.ty, glyim_core::primitives::Mutability::Mut, expr.span);
                let place = glyim_mir::Place::new(local);
                self.push_stmt(glyim_mir::StatementKind::Assign(place.clone(), rvalue), expr.span);
    ...
    pub fn lower_expr_to_place(&mut self, expr: &thir::Expr) -> glyim_mir::Place {
        match &expr.kind {
            ...
            thir::ExprKind::Field { receiver, field, ty: _field_ty } => {
                let base_place = self.lower_expr_to_place(receiver);
                let field_idx = self.resolve_field_index(receiver.ty, *field, expr.span);
                let field_idx = match field_idx { Some(idx) => idx, None => FieldIdx::from_raw(0) };
            ...
            thir::ExprKind::Index { base, index } => {
                ...
                let index_rval = self.lower_expr_to_rvalue(index);
                self.push_stmt(
                    glyim_mir::StatementKind::Assign(glyim_mir::Place::new(index_local), index_rval), index.span,
                );
                self.place_with_projection(base_place, ProjectionElem::Index(index_local))
            }
            thir::ExprKind::Ref { operand, mutability: _ } => self.lower_expr_to_place(operand),
            _ => {
                let rvalue = self.lower_expr_to_rvalue(expr);
                let local = self.alloc_local(expr.ty, glyim_core::primitives::Mutability::Mut, expr.span);
                let place = glyim_mir::Place::new(local);
    ...
    // ---- Pattern binding ----
    pub fn bind_pattern(&mut self, pat: &thir::Pattern, init_local: Option<LocalIdx>, span: glyim_span::Span) {
        match &pat.kind {
            ...
            thir::PatternKind::Tuple(fields) => {
                if let Some(init) = init_local {
                    ...
                    for (idx, field_pat) in fields.iter().enumerate() {
                        ...
                        let field_place = self.place_with_projection(init_place.clone(), field_proj);
            ...
            thir::PatternKind::Struct { adt_id, variant_idx, fields, rest: _rest } => {
                if let Some(init) = init_local {
                    let init_place = glyim_mir::Place::new(init);
                    for field_pat in fields {
                        let field_idx = self.ctx.field_index_by_name(*adt_id, *variant_idx, field_pat.field);
                        let field_idx = match field_idx { Some(idx) => idx, None => continue };
                        let field_proj = ProjectionElem::Field(field_idx);
                        let field_place = self.place_with_projection(init_place.clone(), field_proj);
                        let temp_local = self.alloc_local(field_pat.pattern.ty, glyim_core::primitives::Mutability::Not, field_pat.span);
                        self.push_stmt(glyim_mir::StatementKind::StorageLive(temp_local), field_pat.span);
                        self.push_stmt(
                            glyim_mir::StatementKind::Assign(
                                glyim_mir::Place::new(temp_local),
                                glyim_mir::Rvalue::Use(glyim_mir::Operand::Copy(field_place)),
    ...
    // ---- Literal lowering ----
    fn lower_literal(&self, lit: &thir::Literal, ty: Ty, span: glyim_span::Span) -> glyim_mir::MirConst {
        match lit {
            thir::Literal::Int(val, _) => glyim_mir::MirConst { kind: glyim_mir::MirConstKind::Int(*val), ty, span },
            thir::Literal::Uint(val, _) => glyim_mir::MirConst { kind: glyim_mir::MirConstKind::Uint(*val), ty, span },
            thir::Literal::FloatBits(val, _fty) => glyim_mir::MirConst { kind: glyim_mir::MirConstKind::FloatBits(*val), ty, span },
            thir::Literal::Bool(val) => glyim_mir::MirConst { kind: glyim_mir::MirConstKind::Bool(*val), ty, span },
            thir::Literal::Char(ch) => glyim_mir::MirConst { kind: glyim_mir::MirConstKind::Int(*ch as i128), ty, span },
            thir::Literal::String(name) => glyim_mir::MirConst { kind: glyim_mir::MirConstKind::String(*name), ty, span },
            thir::Literal::Unit => glyim_mir::MirConst { kind: glyim_mir::MirConstKind::Unit, ty, span },
        }
    }

    // ---- Match lowering ----
    fn lower_match(&mut self, scrutinee: &thir::Expr, arms: &[thir::MatchArm], result_ty: Ty, span: glyim_span::Span) -> glyim_mir::Rvalue {
        ...
        for (i, (arm_bb, arm)) in arm_blocks.iter().enumerate() {
            ...
            if let Some(guard) = &arm.guard {
                ...
                let next_arm_bb = if i + 1 < arm_blocks.len() { arm_blocks[i + 1].0 } else { otherwise_bb };

                let guard_targets = glyim_mir::SwitchTargets::new(Box::new([(1, arm_body_bb)]), next_arm_bb);
                self.terminate(
                    glyim_mir::TerminatorKind::SwitchInt {
                        discr: guard_op,
                        switch_ty: guard.ty,
    ...
    fn collect_switch_values(&self, pat: &thir::Pattern, targets: &mut Vec<(u128, BasicBlockIdx)>, arm_bb: BasicBlockIdx) {
        match &pat.kind {
            ...
            thir::PatternKind::Range { start, end, inclusive } => {
                if let (Some(s), Some(e)) = (start, end) {
                    if let (Some(s_val), Some(e_val)) = (self.literal_to_u128(s), self.literal_to_u128(e)) {
                        let end_val = if *inclusive { e_val } else { e_val.saturating_sub(1) };
    ...
    fn lower_arm_body(&mut self, arm: &thir::MatchArm, dest_place: &glyim_mir::Place, merge_bb: BasicBlockIdx) {
        let arm_val = self.lower_expr_to_rvalue(&arm.body);
        self.push_stmt(glyim_mir::StatementKind::Assign(dest_place.clone(), arm_val), arm.body.span);
        self.terminate(glyim_mir::TerminatorKind::Goto { target: merge_bb }, arm.body.span);
    }

    // ---- Field resolution helpers ----
    fn resolve_field_index(&self, receiver_ty: Ty, field_name: glyim_core::interner::Name, _span: glyim_span::Span) -> Option<FieldIdx> {
        match self.ctx.ty_ctx().ty_kind(receiver_ty) {
            ...
            TyKind::Tuple(_substs) => {
                ...
                name_str.parse::<u32>().ok().map(|idx| FieldIdx::from_raw(idx))
            }
            _ => None,
        }
    }
    ...
    // ---- Place helpers ----
    fn place_with_projection(&self, base: glyim_mir::Place, elem: ProjectionElem) -> glyim_mir::Place {
        let mut proj = base.projection.to_vec();
        proj.push(elem);
        glyim_mir::Place { local: base.local, projection: proj.into_boxed_slice() }
    }
}
