        ...
        match &pat.kind {
            ...
            thir::PatternKind::Slice { prefix, slice, suffix } => {
                if let Some(init) = init_local {
                    ...
                    // Get length of the array/slice
                    let len_local = self.alloc_local(
                        glyim_type::Ty::USIZE,
                        glyim_core::primitives::Mutability::Not,
                        span,
                    );
                    self.push_stmt(glyim_mir::StatementKind::StorageLive(len_local), span);
                    let len_rvalue = glyim_mir::Rvalue::Len(init_place.clone());
                    self.push_stmt(
                        glyim_mir::StatementKind::Assign(glyim_mir::Place::new(len_local), len_rvalue),
                        span,
                    );

                    // Bind prefix elements (from start)
                    for (i, elem_pat) in prefix.iter().enumerate() {
                        let idx_local = self.alloc_local(
                            glyim_type::Ty::USIZE,
                            glyim_core::primitives::Mutability::Not,
                            span,
                        );
                        self.push_stmt(glyim_mir::StatementKind::StorageLive(idx_local), span);
                        let idx_const = glyim_mir::MirConst {
                            kind: glyim_mir::MirConstKind::Uint(i as u128),
                            ty: glyim_type::Ty::USIZE,
                            span,
                        };
                        self.push_stmt(
                            glyim_mir::StatementKind::Assign(
                                glyim_mir::Place::new(idx_local),
                                glyim_mir::Rvalue::Use(glyim_mir::Operand::Constant(idx_const)),
                            ),
                            span,
                        );
                        let elem_proj = self.place_with_projection(
                            init_place.clone(),
                            glyim_mir::ProjectionElem::Index(idx_local),
                        );
                        let temp_local = self.alloc_local(
                            elem_pat.ty,
                            glyim_core::primitives::Mutability::Not,
                        ...
                        self.push_stmt(
                            glyim_mir::StatementKind::Assign(
                                glyim_mir::Place::new(temp_local),
                                glyim_mir::Rvalue::Use(glyim_mir::Operand::Copy(elem_proj)),
                            ),
                            span,
                        );
                        self.bind_pattern(elem_pat, Some(temp_local), span);
                    }

                    // Bind the subslice (if present) using a proper subslice projection
                    if let Some(slice_pat) = slice {
                        // Calculate start index = prefix.len()
                        let start_local = self.alloc_local(
                            glyim_type::Ty::USIZE,
                            glyim_core::primitives::Mutability::Not,
                            span,
                        );
                        self.push_stmt(glyim_mir::StatementKind::StorageLive(start_local), span);
                        let start_const = glyim_mir::MirConst {
                            kind: glyim_mir::MirConstKind::Uint(prefix.len() as u128),
                            ty: glyim_type::Ty::USIZE,
                            span,
                        };
                        self.push_stmt(
                            glyim_mir::StatementKind::Assign(
                                glyim_mir::Place::new(start_local),
                                glyim_mir::Rvalue::Use(glyim_mir::Operand::Constant(start_const)),
                            ),
                            span,
                        );

                        // End index = len() - suffix.len()
                        let end_local = self.alloc_local(
                            glyim_type::Ty::USIZE,
                            glyim_core::primitives::Mutability::Not,
                            span,
                        );
                        self.push_stmt(glyim_mir::StatementKind::StorageLive(end_local), span);
                        let end_expr = glyim_mir::Rvalue::BinaryOp(
                            glyim_core::primitives::BinOp::Sub,
                            Box::new((
                                glyim_mir::Operand::Move(glyim_mir::Place::new(len_local)),
                                glyim_mir::Operand::Constant(glyim_mir::MirConst {
                                    kind: glyim_mir::MirConstKind::Uint(suffix.len() as u128),
                                    ty: glyim_type::Ty::USIZE,
                                    span,
                                }),
                            )),
                        ...
                        self.push_stmt(
                            glyim_mir::StatementKind::Assign(glyim_mir::Place::new(end_local), end_expr),
                            span,
                        );

                        // Create a subslice projection: this requires a ProjectionElem::Subslice,
                        // but MIR doesn't have that yet. We'll simulate by creating a temporary
                        // slice using the start and end indices via a new Rvalue::Slice (which also doesn't exist).
                        // As a workaround, we allocate a new local and copy the range using a loop
                        // of individual element copies. For now, we copy the entire original
                        // into a temporary and use that as the slice binding.
                        // A proper implementation would add ProjectionElem::Subslice to glyim_mir.
                        let temp_local = self.alloc_local(
                            slice_pat.ty,
                            glyim_core::primitives::Mutability::Not,
                            span,
                        ...
                        self.push_stmt(
                            glyim_mir::StatementKind::Assign(
                                glyim_mir::Place::new(temp_local),
                                glyim_mir::Rvalue::Use(glyim_mir::Operand::Move(init_place.clone())),
                            ),
                            span,
                        );
                        self.bind_pattern(slice_pat, Some(temp_local), span);
                    }

                    // Bind suffix elements from the end
                    let suffix_len = suffix.len();
                    for (i, elem_pat) in suffix.iter().enumerate() {
                        let idx_local = self.alloc_local(
                            glyim_type::Ty::USIZE,
                            glyim_core::primitives::Mutability::Not,
                            span,
                        ...
                        self.push_stmt(glyim_mir::StatementKind::StorageLive(idx_local), span);
                        let idx_expr = glyim_mir::Rvalue::BinaryOp(
                            glyim_core::primitives::BinOp::Sub,
                            Box::new((
                                glyim_mir::Operand::Move(glyim_mir::Place::new(len_local)),
                                glyim_mir::Operand::Constant(glyim_mir::MirConst {
                                    kind: glyim_mir::MirConstKind::Uint((suffix_len - i) as u128),
                                    ty: glyim_type::Ty::USIZE,
                                    span,
                                }),
                            )),
                        ...
                        self.push_stmt(
                            glyim_mir::StatementKind::Assign(glyim_mir::Place::new(idx_local), idx_expr),
                            span,
                        );
                        let elem_proj = self.place_with_projection(
                            init_place.clone(),
