...
fn unicode_error_in_middle() {
    let result = lex_result("fn \u{00A9} main()", FileId::from_raw(0));
...
#[test]
fn unicode_at_start() {
    let result = lex_result("\u{00A9}fn", FileId::from_raw(0));
...
#[test]
fn unicode_at_end() {
    let result = lex_result("fn\u{00A9}", FileId::from_raw(0));
...
fn multiple_unicode_errors() {
    let result = lex_result("\u{00A9}\u{00AE}\u{00B6}", FileId::from_raw(0));
...
#[test]
fn emoji_in_source() {
    let result = lex_result("\u{1F600}", FileId::from_raw(0));
...
fn null_byte_produces_error() {
    let result = lex_result("\0", FileId::from_raw(0));
    assert_eq!(result.tokens.len(), 1);
    assert_eq!(result.tokens[0].kind, SyntaxKind::Error);
    assert!(!result.diagnostics.is_empty());
}
...
fn null_byte_between_tokens() {
    let result = lex_result("1\0 2", FileId::from_raw(0));
    let kinds: Vec<_> = result.tokens.iter().map(|t| t.kind).collect();
...
fn del_character_produces_error() {
    let result = lex_result("\u{007F}", FileId::from_raw(0));
    assert_eq!(result.tokens.len(), 1);
    assert_eq!(result.tokens[0].kind, SyntaxKind::Error);
}
...
fn replacement_character_produces_error() {
    let result = lex_result("\u{FFFD}", FileId::from_raw(0));
    assert_eq!(result.tokens.len(), 1);
    assert_eq!(result.tokens[0].kind, SyntaxKind::Error);
}
...
fn byte_order_mark_produces_error() {
    let result = lex_result("\u{FEFF}", FileId::from_raw(0));
    assert_eq!(result.tokens.len(), 1);
    assert_eq!(result.tokens[0].kind, SyntaxKind::Error);
}
...
fn mixed_ascii_and_unicode_errors() {
    let result = lex_result("fn \u{00A9} x + \u{00AE} 1", FileId::from_raw(0));
    let kinds: Vec<_> = result.tokens.iter().map(|t| t.kind).collect();
...
fn cjk_character_produces_error() {
    let result = lex_result("\u{4E16}", FileId::from_raw(0));
    assert_eq!(result.tokens.len(), 1);
    assert_eq!(result.tokens[0].kind, SyntaxKind::Error);
}
...
fn right_to_left_mark_produces_error() {
    let result = lex_result("\u{200F}", FileId::from_raw(0));
    assert_eq!(result.tokens.len(), 1);
    assert_eq!(result.tokens[0].kind, SyntaxKind::Error);
}
...
fn zero_width_space_produces_error() {
    let result = lex_result("\u{200B}", FileId::from_raw(0));
    assert_eq!(result.tokens.len(), 1);
    assert_eq!(result.tokens[0].kind, SyntaxKind::Error);
}
...
fn unicode_error_recovery_preserves_adjacent_tokens() {
    let result = lex_result("let \u{00A9} = 42;", FileId::from_raw(0));
