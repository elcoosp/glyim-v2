...
fn parse_type(src: &str) -> SyntaxNode {
    ...
    let type_alias = parse.root.children().find(|n| n.kind() == SyntaxKind::TypeAlias)
        .expect("TypeAlias node not found");
    type_alias.children().find(|n| is_type_node(n))
...
fn test_lower_dyn_type() {
    ...
    let mut interner = Interner::default();
    let ty = lower_type_ref(&node, &mut interner).unwrap();
    // For dyn, HIR currently lowers to TypeRef::Path (the trait name)
    match ty {
        TypeRef::Path(path) => {
...
fn test_lower_fn_ptr_type() {
    ...
    match ty {
        TypeRef::Fn { params, ret } => {
            ...
            // Accept any type for params, just ensure they are present
            assert!(ret.is_some(), "Return type should exist");
        }
