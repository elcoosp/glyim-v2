...
use glyim_test::{assert_ty, check_ty, test_frozen_ty_ctx, with_fresh_ty_ctx, TyFactory};
use glyim_type::*;

fn make_projection(ctx: &mut TyCtxMut, trait_def_id: TraitDefId, self_ty: Ty, item_name: &str) -> Ty {
    let name = ctx.resolver().intern(item_name);
    let substs = ctx.intern_substitution(vec![GenericArg::Ty(self_ty)]);
    let trait_ref = TraitRef { def_id: trait_def_id, substs };
    let proj = ProjectionTy { trait_ref, item_name: name };
...
fn projection_equality() {
    let (ctx, (p1, p2, p3)) = with_fresh_ty_ctx(|ctx| {
        ...
        let proj1 = ctx.mk_ty(TyKind::Projection(ProjectionTy { trait_ref: trait_ref.clone(), item_name: item }));
        let proj2 = ctx.mk_ty(TyKind::Projection(ProjectionTy { trait_ref: trait_ref.clone(), item_name: item }));
        let proj3 = ctx.mk_ty(TyKind::Projection(ProjectionTy { trait_ref: trait_ref, item_name: ctx.resolver().intern("Other") }));
        (proj1, proj2, proj3)
    });
    assert_eq!(p1, p2);
    assert_ne!(p1, p3);
