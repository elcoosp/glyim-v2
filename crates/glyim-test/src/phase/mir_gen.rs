...
impl MirGenTester {
    pub fn lower_body(
        ...
        if result.diagnostics.is_empty() { Ok(result.body) } else { Err(result.diagnostics) }
    }
    ...
    pub fn optimize(ctx: &glyim_type::TyCtx, body: &std::sync::Arc<glyim_mir::Body>) -> glyim_mir::Body {
