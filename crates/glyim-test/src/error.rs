...
impl std::fmt::Display for FailureReason {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::RunFailed { exit_code, expected_exit_code } => {
                write!(f, "run failed: exit code {:?}, expected {:?}", exit_code, expected_exit_code)
            }
            Self::StdoutMismatch { expected, actual } => {
                write!(f, "stdout mismatch:\n  expected: {:?}\n  actual:   {:?}", expected, actual)
            }
            Self::StderrMismatch { expected, actual } => {
                write!(f, "stderr mismatch:\n  expected: {:?}\n  actual:   {:?}", expected, actual)
            }
