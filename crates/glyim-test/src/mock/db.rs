...
impl TestDbBuilder {
    pub fn new() -> Self {
        Self { name: None, target_triple: None, opt_level: 0, files: Vec::new() }
    }
    pub fn name(mut self, name: impl Into<String>) -> Self { self.name = Some(name.into()); self }
    pub fn target_triple(mut self, triple: impl Into<String>) -> Self { self.target_triple = Some(triple.into()); self }
    pub fn opt_level(mut self, level: u8) -> Self { self.opt_level = level; self }
    ...
    pub fn build(self) -> Database {
        let config = CrateConfig {
            ...
            target_triple: self.target_triple.unwrap_or_else(|| "x86_64-unknown-linux-gnu".to_string()),
