...
fn vtable_size_and_alignment() {
    ...
    assert_eq!(layout.size, Size::bytes(ptr_size.0 * 2), "size = two pointers");
    assert_eq!(layout.align, computer.ptr_align(), "alignment = pointer alignment");
    assert!(!layout.is_unsized, "dyn Trait fat pointer is sized");
    match &layout.fields {
        FieldsShape::Arbitrary { offsets } => {
            assert_eq!(offsets.len(), 2, "two fields");
            assert_eq!(offsets[FieldIdx::from_raw(0)], Size::ZERO, "data ptr at offset 0");
            assert_eq!(offsets[FieldIdx::from_raw(1)], Size::bytes(ptr_size.0), "vtable ptr at offset ptr_size");
        }
