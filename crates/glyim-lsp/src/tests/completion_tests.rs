...
fn setup_test_db() -> (AnalysisDatabase, FileId, PathBuf) {
    ...
    }
    {
        ...
        sm.insert(file_id, SourceMap::new(path.clone(), file_id, "struct Point { x: i32, y: i32 }\nfn main() { let p = Point { x: 10, y: 20 }; p. }".to_string()));
    }
    {
        ...
        idx.insert_test_symbol(file_id, SymbolInfo {
            ...
            definition: DefinitionLocation { file_id, span: make_span(file_id, 14, 15) },
            type_signature: Some(TypeSignature { params: vec![], return_type: Some("i32".into()) }),
            is_pub: true,
            documentation: None,
        });
        idx.insert_test_symbol(file_id, SymbolInfo {
            name: "y".into(),
            kind: SymbolKind::Field,
            definition: DefinitionLocation { file_id, span: make_span(file_id, 24, 25) },
            type_signature: Some(TypeSignature { params: vec![], return_type: Some("i32".into()) }),
            is_pub: true,
            documentation: None,
        });
...
fn completion_provides_struct_fields() {
    ...
    let params = CompletionParams {
        text_document_position: TextDocumentPositionParams {
            ...
            position: Position { line: 1, character: 42 },
        },
