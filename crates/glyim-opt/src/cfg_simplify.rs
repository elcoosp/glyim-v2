...
/// via a single unconditional Goto edge. Uses a single pass with predecessor map.
pub(crate) fn run(_ctx: &TyCtx, body: &mut Body) {
    let mut blocks: Vec<BasicBlockData> = std::mem::take(&mut body.basic_blocks).into_raw();
    ...
    // Single pass: merge blocks that have exactly one predecessor which is a single-successor Goto
    for bb in 1..blocks.len() {
        ...
        // Update predecessor map for successors of the merged block
        for succ in terminator_successors(&blocks[pred].terminator) {
            let si = succ.to_raw() as usize;
            if si < blocks.len() {
                preds[si].push(pred);
            }
