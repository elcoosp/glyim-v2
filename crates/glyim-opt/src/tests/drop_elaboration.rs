...
fn body_with_drop(ctx: &TyCtx, local_ty: Ty) -> Body {
...
fn body_with_array_drop(ctx: &TyCtx, elem_ty: Ty, len: u64) -> Body {
    let mut body = Body::dummy(DefId::from_raw(0, 0));
    let array_ty = ctx.mk_ty(TyKind::Array(elem_ty, Const::from_usize(ctx, len)));
...
fn body_with_enum_drop(ctx: &TyCtx, enum_ty: Ty) -> Body {
    let mut body = Body::dummy(DefId::from_raw(0, 0));
    let local = body.locals.push(LocalDecl {
        ty: enum_ty,
        mutability: Mutability::Mut,
...
fn conditional_drop_after_partial_move() {
    ...
    let ty = ctx.mk_ty(TyKind::Adt(AdtId::from_raw(0), Substitution::empty()));
    let mut body = body_with_drop(&ctx, ty);
...
fn array_drop_reverse() {
    let ctx = test_frozen_ty_ctx();
    let elem_ty = ctx.mk_ty(TyKind::Adt(AdtId::from_raw(0), Substitution::empty()));
    let mut body = body_with_array_drop(&ctx, elem_ty, 3);
    crate::elaborate_drops(&ctx, &mut body);
...
fn enum_drop_glue_discriminant() {
    ...
    let enum_ty = ctx.mk_ty(TyKind::Adt(AdtId::from_raw(1), Substitution::empty()));
    let mut body = body_with_enum_drop(&ctx, enum_ty);
    crate::elaborate_drops(&ctx, &mut body);
