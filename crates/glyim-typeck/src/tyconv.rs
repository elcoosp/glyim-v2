...
fn resolve_path_type(
    ...
    // Multi-segment: use the def map resolver
    ...
    diagnostics.push(GlyimDiagnostic::type_error(
        ...
        format!(
            ...
            path.segments.iter().map(|s| ctx.name_str(s.name)).collect::<Vec<_>>().join("::")
        ),
    ));
    Ty::ERROR
}
...
fn resolve_name_to_adt_ty(ctx: &mut TyCtxMut, def_map: &glyim_def_map::CrateDefMap, name: Name) -> Option<Ty> {
    let res = def_map.modules[def_map.root].scope.resolve(name)?;
    let adt_id = AdtId::from_raw(res.0.to_raw());
    let substs = ctx.intern_substitution(vec![]);
