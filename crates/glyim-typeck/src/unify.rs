...
impl<'a> FnCtxt<'a> {
    ...
    pub fn check_path(&mut self, path: &Path, span: Span) -> (thir::Expr, Ty) {
        ...
        // If single-segment and we got here, it's an unresolved name
        if let Some(name) = path.as_name() {
            self.diagnostics.push(GlyimDiagnostic::type_error(
                span,
        ...
        self.diagnostics.push(GlyimDiagnostic::type_error(
            ...
            format!(
                "unresolved path: {}",
                path.segments.iter().map(|s| self.ctx.name_str(s.name)).collect::<Vec<_>>().join("::")
            ),
        ));
    ...
    pub fn instantiate_fn_sig(&mut self, def_id: FnDefId, span: Span) -> Ty {
        ...
        for (_id, item) in self.hir.items.iter_enumerated() {
            if let glyim_hir::ItemKind::Fn(fn_item) = &item.kind {
                // Match by raw id — the FnDefId encodes the position
        ...
        // Fallback: also check impl methods
        for (_id, item) in self.hir.items.iter_enumerated() {
