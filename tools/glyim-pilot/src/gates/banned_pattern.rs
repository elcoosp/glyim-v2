        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            if use_diff {
                ...
                for rel_path in &changed_files {
                    ...
                    // Skip test directories (optional)
...
/// Strip all string literals, raw strings, byte strings, character literals,
/// and line comments from a line of Rust code to avoid false positives.
...
mod tests {
    ...
    fn test_strip_raw_string() {
        // Use raw string literal for test input to avoid double escapes
    ...
    fn test_strip_raw_string_hash() {
        // The raw string with hash delimiter: use a raw string with a different delimiter (r##)
        let input = r##"let s = r#"contains { braces }"#; let y = foo;"##;
        assert_eq!(
            strip_string_literals(input),
            "let s = ; let y = foo;"
    ...
    fn test_strip_byte_raw_string() {
        // Use r## delimiter for outer raw string to avoid conflict
        let input = r##"let b = br#"contains { braces }"#; let y = foo;"##;
        assert_eq!(
            strip_string_literals(input),
            "let b = ; let y = foo;"
        );
