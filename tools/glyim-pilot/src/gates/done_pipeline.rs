...
use crate::gates::{Gate, PipelineResult, dead_code::DeadCodeGate, coverage::CoverageGate, mutation::MutationGate, workspace_check::WorkspaceCheckGate, audit::AuditGate};
use std::sync::Arc;
...
pub async fn run_done_pipeline(ctx: &GateContext, config: &ResolvedDoneGates) -> Result<PipelineResult, PilotError> {
    ...
    if gates.is_empty() { return Ok(PipelineResult::from_gates(vec![crate::gates::types::GateResult::pass("done_pipeline")])); }
