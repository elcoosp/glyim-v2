# Stream M06: Split MIR Builder (glyim-lower/lower.rs)

## Mission
Implement Split `crates/glyim-lower/src/lower.rs` (600+ lines) into `builder.rs`, `lower_rvalue.rs`, `lower_terminator.rs`. Keep `MirBuilder` and `lower_body` public. for the Glyim compiler.
...
## Mocking Strategy
Use `MockLowerCtx` from `glyim-test` to verify builder behaviour.
...
- [ ] PR created against `main` with branch `stream-M06/v0.1.0`
