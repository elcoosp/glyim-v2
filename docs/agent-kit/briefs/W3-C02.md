# Stream W3-C02: LLVM debug info for macro locations

## Mission
Implement Integrate `HygieneCtx` into `LlvmBackend`. Implement `resolve_span_to_location` to walk back macro expansions. Emit correct `DILocation` for statements in expanded code. for the Glyim compiler.
...
## Mocking Strategy
Inspect `!DILocation` metadata in LLVM IR.
