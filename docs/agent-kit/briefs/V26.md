# Stream SV26: Post‑Monomorphization Checks

## Mission
Implement Run lint‑like checks after monomorphization: detect unused generic parameters (polymorphisation light), detect mono item size blow‑up, and emit errors for unsized locals in instantiated functions. for the Glyim compiler.
...
## Mocking Strategy
Requires MIR after instantiation. Use `glyim_test::assert_diag_contains`.
...
- [ ] PR created against `main` with branch `stream-V26/v0.1.0`
