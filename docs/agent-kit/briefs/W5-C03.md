# Stream W5-C03: Bytecode backend: OP_INDEX, OP_FIELD, OP_REPEAT, remove stubs

## Mission
Implement In `lib.rs`, implement `OP_INDEX` and `OP_FIELD` in `emit_place_address` (use element size multiplication for index). Implement `OP_REPEAT` for `Rvalue::Repeat`. Remove stubs for out‑of‑bounds locals and zero‑sized elements – replace with `panic!`. Ensure `emit_rvalue` for `Aggregate` works for all kinds. for the Glyim compiler.
...
- [ ] PR created against `main` with branch `stream-W5-C03/v0.1.0`
