# Stream S10: Def-Map & Name Resolution Unstubbing

## Mission
Implement Remove all `stub!()` and `stub_impl!()` calls in `glyim-def-map`. Implement `build_def_map` to walk AST, build `ModuleData` tree, `ItemScope::declare`, `Resolver::resolve_path`, visibility checking, and `ModuleOrigin` tracking. Change the `build_def_map` signature to accept an `&Interner` parameter for symbol resolution. for the Glyim compiler.

## What You Own
- glyim-def-map

## What You May NOT Modify
Any `pub` interface listed in `CONTRACTS_LOCKED.md` for crates you do NOT own.
Specifically, you MUST NOT change:
- glyim_def_map::build_def_map
- glyim_def_map::CrateDefMap
- glyim_def_map::Resolver

## What You May Do Freely
- Add `pub(crate)` helper functions in crates you own
- Add new private modules in crates you own
- Add new `pub` items in NEW modules you create within crates you own
- Add dev-dependencies for testing

## Test Location (MANDATORY)

**ALL tests MUST be written as unit tests in `src/tests/` within the crate.**

NEVER use the `tests/` directory at the crate root (integration tests). Unit tests compile faster because they share the crate's private API.

### Setup Pattern

1. In `crates/glyim-def-map/src/lib.rs`, add:
   ```rust
   #[cfg(test)]
   mod tests;
   ```

2. Create `crates/glyim-def-map/src/tests/mod.rs` declaring submodules:
   ```rust
   mod module_a;
   mod module_b;
   ```

3. Each test file contains `#[test]` functions with full access to `pub(crate)` items.

4. Add `glyim-test` as a dev-dependency:
   ```toml
   [dev-dependencies]
   glyim-test = { workspace = true }
   ```

5. Read `docs/agent-kit/GLYIM_TEST_INSTRUCTIONS.md` for full API reference.

## Test Plan (Write These FIRST)
- S10-T01: build_def_map constructs correct module hierarchy
- S10-T02: Resolver::resolve_path finds items in parent modules
- S10-T03: Visibility checks prevent access to private items

## Mocking Strategy
Use `glyim_test::snapshot_def_map`.

## Implementation Priority
1. Create `src/tests/` module structure and wire up in `lib.rs`
2. Write ALL tests above — verify they compile with `cargo check -p glyim-def-map`
3. Implement until all tests pass
4. Remove all `todo!()` and silent stubs from non-test code
5. Run `cargo clippy -p glyim-def-map` — fix all warnings
6. Run `cargo fmt`

## Verification Commands
```bash
cargo test -p glyim-def-map
cargo clippy -p glyim-def-map -- -D warnings
cargo fmt --check -p glyim-def-map
cargo check --workspace
```

## Definition of Done
- [ ] All test cases pass
- [ ] Tests are in `src/tests/` (NOT `tests/`)
- [ ] Zero `todo!()` in non-test code
- [ ] Zero clippy warnings
- [ ] `cargo fmt` applied
- [ ] `cargo check --workspace` succeeds
- [ ] PR created against `main` with branch `stream-S10/v0.1.0`

## Dependencies on Other Streams
- S05
- S02

## Downstream Consumers
- S16
