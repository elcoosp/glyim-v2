# Stream W1-C01: Slice projection codegen

## Mission
Implement Implement `ProjectionElem::Slice` in `emit_place_address` and `emit_operand`. Compute fat pointer `{ data, len }` for array/slice subslicing. Handle zero-element slices. Use element size for indexing. for the Glyim compiler.
...
## Mocking Strategy
Use `MockCodegen` to capture generated IR.
