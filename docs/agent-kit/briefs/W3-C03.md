# Stream W3-C03: HIR field access completeness

## Mission
Implement Ensure `Expr::Field` lowering uses `TyCtx::field_ty` to correctly resolve field types. Add support for tuple field access (`.0`, `.1`). Remove any remaining stubs or warnings in lower_expr for field access. for the Glyim compiler.
...
## Mocking Strategy
Parse and lower, inspect HIR types.
...
- [ ] PR created against `main` with branch `stream-W3-C03/v0.1.0`
