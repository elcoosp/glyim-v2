# Stream SU06: LLVM Backend – Operators & Casts

## Mission
Implement Remove stubs for binary and unary ops on floats, `Not` on float, `Deref` on float, and `Cast` kinds (IntToFloat, FloatToInt, PtrToPtr). Implement all missing float arithmetic and comparisons. for the Glyim compiler.
