# Stream W5-C05: Runtime: TCP networking

## Mission
Implement Implement TCP functions: `glyim_net_tcp_connect`, `bind`, `accept`, `read`, `write`, `local_addr`. Use `std::net::TcpStream` and `TcpListener`. Maintain a global socket handle registry. Return negative error codes. for the Glyim compiler.
...
- [ ] PR created against `main` with branch `stream-W5-C05/v0.1.0`
