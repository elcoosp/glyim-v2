# Stream M07: Split Polymorphization (glyim-lower/polymorphize.rs)

## Mission
Implement Split `crates/glyim-lower/src/polymorphize.rs` (550+ lines) into `analyze.rs`, `substitute.rs`, `dedup.rs`. Keep `polymorphize_substs`, `compute_poly_item`, and `deduplicate` public. for the Glyim compiler.
...
## Mocking Strategy
Use `glyim_test::mock::MockLowerCtx`; run existing tests.
...
- [ ] PR created against `main` with branch `stream-M07/v0.1.0`
