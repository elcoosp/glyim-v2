# Stream M01: Split LLVM Backend (glyim-codegen-llvm)

## Mission
Implement Split `crates/glyim-codegen-llvm/src/lib.rs` (1300+ lines) into submodules: `lower/` (MIR→LLVM lowering), `types/` (type mapping), `passes/` (optimisation pipeline), `debug/` (DWARF generation). Keep public API unchanged. Ensure all tests pass. for the Glyim compiler.
...
## Mocking Strategy
No mocking – pure refactoring. Use existing test suite to verify correctness.
...
- [ ] PR created against `main` with branch `stream-M01/v0.1.0`
