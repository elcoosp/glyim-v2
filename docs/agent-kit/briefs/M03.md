# Stream M03: Split HIR Lowering (glyim-hir)

## Mission
Implement Split `crates/glyim-hir/src/lower.rs` (800+ lines) into `lower_item.rs`, `lower_expr.rs`, `lower_pat.rs`, `lower_type.rs`. Each file handles one category of AST → HIR conversion. Keep `lower_crate` public entry point unchanged. for the Glyim compiler.
...
## Mocking Strategy
Use existing HIR unit tests; no mocking required.
...
- [ ] PR created against `main` with branch `stream-M03/v0.1.0`
