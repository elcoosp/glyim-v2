# Stream W1-C03: Const evaluator for patterns

## Mission
Implement New crate `glyim_const_eval` with HIR‑based constant evaluator. Supports literals, arithmetic, `if`, `match`. Called during HIR→THIR lowering to replace `ConstBlock` patterns with literals. for the Glyim compiler.
...
1. In `crates/glyim_const_eval/src/lib.rs`, add:
   ```rust
   #[cfg(test)]
   mod tests;
   ```

2. Create `crates/glyim_const_eval/src/tests/mod.rs` declaring submodules:
   ```rust
   mod module_a;
   mod module_b;
   ```
...
## Mocking Strategy
Unit tests on HIR directly.

## Implementation Priority
1. Create `src/tests/` module structure and wire up in `lib.rs`
2. Write ALL tests above — verify they compile with `cargo check -p glyim_const_eval`
3. Implement until all tests pass
4. Remove all `todo!()` and silent stubs from non-test code
5. Run `cargo clippy -p glyim_const_eval` — fix all warnings
6. Run `cargo fmt`

## Verification Commands
```bash
cargo test -p glyim_const_eval
cargo clippy -p glyim_const_eval -- -D warnings
cargo fmt --check -p glyim_const_eval
